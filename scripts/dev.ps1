...
$ui = Start-Process -FilePath "npm" -ArgumentList "--prefix", "crates/golden_ui", "run", "dev" -WorkingDirectory $workspace -PassThru

Write-Host "Starting Tauri app (auto-rebuild on Rust changes)..."
$tauri = Start-Process -FilePath "cargo" -ArgumentList "watch", "-x", "run -p golden_tauri" -WorkingDirectory $workspace -PassThru

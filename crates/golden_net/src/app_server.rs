...
async fn ws_handler(
    ...
    ws.on_upgrade(move |socket| handle_socket(state.engine, socket))
}

async fn handle_socket(engine: Arc<Mutex<Engine>>, socket: WebSocket) {
    ...
    if send_snapshot(&out_tx, snapshot).is_err() {
        return;
    }
